	...
	turn(*this, "Slay, slay!")
{
	...
	// Dialogs
	turn.set_secondary_text("Let us see if you remember the order at which Simon slayed the animals.");
...
bool SimonSlays::simonMove(std::list<int>& someList){
	...
	if(holder != -1){
		switch (holder){
		case 0:
			gameButton[holder].set_image(goat);
			break;
		case 1:
			gameButton[holder].set_image(chicken);
			break;
		case 2:
			gameButton[holder].set_image(pig);
			break;
		case 3:
			gameButton[holder].set_image(cow);
			break;
		};
	}
	...
	switch(first){
	case 0:
		gameButton[first].set_image(goatclicked);
		break;
	case 1:
		gameButton[first].set_image(chickenclicked);
		break;
	case 2:
		gameButton[first].set_image(pigclicked);
		break;
	case 3:
		gameButton[first].set_image(cowclicked);
		break;
	};
